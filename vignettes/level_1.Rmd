...
embed_png <- function(path,
  ...
  knitr::asis_output(paste0("<img src='",
                            path,
                            "'",
                            " width=",
                            round(x = meta$dim[1] / (meta$dpi[1] / 96)),
                            " height=",
                            round(x = meta$dim[2] / (meta$dpi[2] / 96)),
                            " />"
                            ))
}
```
...
|--------------------------------------|--------------------------------|----------------------|--------------|
| Process of Raising Factor level 1    | `rf1()`                        | full_trips           |              |
| Process of Raising Factor level 2    | `rf2()`                        | full_trips           |              |
| Logbook weigth categories conversion | `conversion_weigth_category()` | full_trips           |              |
| Postive sets count                   | `set_count()`                  | full_trips           |              |
| Set duration calculation             | `set_duration()`               | full_trips           | In hours     |
| Time at sea calculation              | `time_at_sea()`                | full_trips           | In hours     |
| Fishing time calculation             | `fishing_time()`               | full_trips           | In hours     |
| Searching time calculation           | `searching_time()`             | full_trips           | In hours     |
...
<img style="display: block; margin-left: auto; margin-right: auto; width: 120%;" src='process_1.1.png'/>
